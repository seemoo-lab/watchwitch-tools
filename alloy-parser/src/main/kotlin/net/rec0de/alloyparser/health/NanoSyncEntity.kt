...
class Device(
    ...
    companion object : PBParsable<Device>() {
        override fun fromSafePB(pb: ProtoBuf): Device {
            ...
            val uuid = if(uuidBytes == null) null else net.rec0de.alloyparser.Utils.uuidFromBytes(uuidBytes)
...
class SourceAuthorization(
    ...
    companion object : PBParsable<SourceAuthorization>() {
        override fun fromSafePB(pb: ProtoBuf): SourceAuthorization {
            val sourceUUID = net.rec0de.alloyparser.Utils.uuidFromBytes((pb.readAssertedSinglet(1) as ProtoLen).value)
            val authorization = Authorization.fromPB(pb.readOptPB(2))
            val backupUUIDBytes = (pb.readOptionalSinglet(3) as ProtoLen?)?.value
            val backupUUID = if(backupUUIDBytes == null) null else net.rec0de.alloyparser.Utils.uuidFromBytes(backupUUIDBytes)
...
class ObjectAssociation(
    ...
    companion object : PBParsable<ObjectAssociation>() {
        override fun fromSafePB(pb: ProtoBuf): ObjectAssociation {
            val associationUUID = net.rec0de.alloyparser.Utils.uuidFromBytes((pb.readAssertedSinglet(1) as ProtoLen).value)
...
class Source(
    ...
    companion object : PBParsable<Source>() {
        override fun fromSafePB(pb: ProtoBuf): Source {
            ...
            val uuid = net.rec0de.alloyparser.Utils.uuidFromBytes((pb.readAssertedSinglet(5) as ProtoLen).value)
