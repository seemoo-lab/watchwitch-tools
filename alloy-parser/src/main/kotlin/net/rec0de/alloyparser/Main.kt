...
import net.rec0de.alloyparser.utun.DataMessage
import net.rec0de.alloyparser.utun.ProtobufMessage
import net.rec0de.alloyparser.utun.UTunControlMessage
import net.rec0de.alloyparser.utun.UTunMessage
import java.io.File
...
fun main(args: Array<String>) {
    ...
    if(args.size != 1) {
        println("Alloy parser expects exactly one argument")
        println("Usage: alloy-parser path/to/ids/logfile")
        return
    }
...
fun readLine(line: String) {
    ...
    if(rest.startsWith("aovercmap ")) {
        rest = rest.removePrefix("aovercmap ")
    ...
    else if(rest.startsWith("aovercrsa ")){
        Decryptor.loadLocalPrivKey(rest.removePrefix("aovercrsa ").hexBytes())
    }
    ...
    if(control)
        ...
        readUTun(incoming, opcode, parts[1].hexBytes())
    }
...
fun readUTun(incoming: Boolean, opcode: Int, bytes: ByteArray) {
