...
import net.rec0de.alloyparser.GzipDecoder
...
): UTunCommonMessage(sequence, streamID, flags, responseIdentifier, messageUUID, topic, expiryDate) {
...
open class AckMessage(sequence: Int) : UTunMessage(sequence) {
    ...
    override fun toString() = "AckMessage(seq $sequence)"
}
...
class AppAckMessage(sequence: Int, val streamID: Int, val responseIdentifier: String?, val topic: String?): UTunMessage(sequence) {
    ...
    override fun toString() = "AppAckMessage(seq $sequence stream $streamID, responseID $responseIdentifier, topic $topic)"
}
...
class ResourceTransferMessage(
    ...
    companion object : UTunParseCompanion() {
        ...
        fun parse(bytes: ByteArray): ResourceTransferMessage {
            ...
            val c = commonData.first
            val rest = commonData.second
            return ResourceTransferMessage(
                c.sequence,
                c.streamID,
                c.flags,
                c.responseIdentifier,
...
class ServiceMapMessage(val streamID: Int, val serviceName: String, val reason: Int): UTunMessage(0) {
    ...
    override fun toBytes(): ByteArray {
        ...
        val msg = ByteBuffer.allocate(5 + length) // 5 bytes net.rec0de.alloyparser.utun type + len
        msg.put(0x27)
        msg.putInt(length)
